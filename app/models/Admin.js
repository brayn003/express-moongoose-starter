...
AdminSchema.statics.addOne = async function ({ user: userId, roles, createdBy = null }) {
  const user = await this.model('User').findById(userId);
  if (!user) {
    throw new Error('User does not exist');
  }
  const admin = await this.create({ user, createdBy, roles });
...
AdminSchema.statics.getAll = async function () {
